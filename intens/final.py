...
button_1 = tkinter.Button(main_window, text='заново', command=new_game, bg='gray', fg='blue')
button_1.config(font=("Courier", 16))
button_1.place(x=0, y=G_HEIGHT - 50)
button_2 = tkinter.Button(main_window, text='выход', command=exit_game, bg='gray', fg='blue')
...
def check_letter(letter):
    if len(letter) == 1 and 1072 <= ord(letter) <= 1103 and letter not in used_letters:
...
def key_press(arg):
    ...
    if used_letters:
        var_mistakes_else.set(f"запас {mistakes_else}, пробовали буквы: {' '.join(used_letters)}")
